...
if (isset($_POST['login'])) {
    ...
    $confirmPassword = sha1($_POST['comfirm_password']);

    // Vérifier si le nom d'utilisateur existe déjà
    $checkUsername = $conn->prepare('SELECT * FROM connection_admin WHERE nom_utilisateur = ?');
    ...
    if ($checkUsername->rowCount() > 0) {
      $error_message = "Ce nom d'utilisateur est déjà utilisé. Veuillez en choisir un autre.";
        ...
        if ($password === $confirmPassword) {
            ...
            if (preg_match('/^(?=.*[A-Z])(?=.*[a-z])(?=.*\d)(?=.*[!@#$%^&*()?-_+=])/', $password_v)) {
                // Ajouter l'utilisateur à la base de données
                $insertUser = $conn->prepare('INSERT INTO connection_admin(nom_utilisateur, mot_de_passe) VALUES (?, ?)');
                $insertUser->execute([$username, $password]);

                // Récupérer l'utilisateur ajouté
                $recupUser = $conn->prepare('SELECT * FROM connection_admin WHERE nom_utilisateur = ? AND mot_de_passe = ?');
...
?>
    ...
    <form method="post" action="" class="formulaire">
        <div class="mb-3">
            ...
            <input type="text" name="username" class="form-control" id="exampleFormControlInput1" placeholder="Entrez votre pseudo" required autocomplete = "off">
        </div>
        <label for="inputPassword5" class="form-label">Mot de passe</label>
        <input type="password" name="password"id="inputPassword5" class="form-control" aria-describedby="passwordHelpBlock" placeholder="Entrez votre mot de passe" required autocomplete = "off">
        <br><label for="inputPassword5" class="form-label">Confirmer votre mot de passe</label>
        <input type="password" name="comfirm_password"id="inputPassword5" class="form-control" aria-describedby="passwordHelpBlock" placeholder="Entrez votre mot de passe" required autocomplete = "off">

        <?php
        if (!empty($error_message)) {
            echo '<br>';
            echo '<div class="erreur" style="color: red;">';
            echo $error_message;
            echo '</div>';
        }
        ?>
